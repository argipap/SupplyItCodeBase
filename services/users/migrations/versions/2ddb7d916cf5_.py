...
def create_enum_type_if_not_exists(enum_type):
    ...
    if context.bind.dialect.name == 'postgresql':
        has_size_type = context.bind.execute(
            f"select exists (select 1 from pg_type  where typname='{enum_type}')").scalar()
...
def upgrade():
    ...
    op.add_column('users', sa.Column('user_type', sa.Enum('retail', 'wholesale', name='usertype'), nullable=True))
