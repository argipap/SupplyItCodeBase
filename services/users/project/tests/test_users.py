...
class TestUserService(BaseTestCase):
    ...
    def test_add_user_invalid_json(self):
        ...
        with self.client:
            ...
            for key in TestData.invalid_payload_error.keys():
                self.assertIn(TestData.invalid_payload_error[key], data['message'][key][0])
            ...
            for key in TestData.invalid_payload_error.keys():
                if key not in data['message'].keys():
                    for invalid_key in data['message'].keys():
                        self.assertIn(TestData.invalid_payload_error[key], data['message'][invalid_key][0])
            self.assertIn('fail', data['status'])
    ...
    def test_all_users(self):
        ...
        with self.client:
            ...
            self.assertIn(
                TestData.user_data_1['email'], data['data'][0]['email'])
            self.assertIn(TestData.user_data_2['username'], data['data'][1]['username'])
            self.assertIn(
                TestData.user_data_2['email'], data['data'][1]['email'])
            self.assertIn('success', data['status'])
    ...
    def test_main_with_users(self):
        ...
        with self.client:
            ...
            self.assertIn(f'{TestData.user_data_1["username"]}'.encode('utf-8'), response.data)
            self.assertIn(f'{TestData.user_data_2["username"]}'.encode('utf-8'), response.data)
